...
from .models import RepositoryEvidenceObject, ValidationReport
    ...
    def validate_repository_evidence(
        ...
        for idx, ev in enumerate(evidence_list):
            if not ev.evidence_id:
                ev.evidence_id = f"ev_repo_{idx+1:04d}"

            if not ev.quote or not ev.quote.strip():
                warnings.append(f"Evidence '{ev.evidence_id}' has empty quote or location.")

            if ev.quote in seen_quotes:
                warnings.append(f"Duplicate repository evidence quote detected for '{ev.evidence_id}'.")
